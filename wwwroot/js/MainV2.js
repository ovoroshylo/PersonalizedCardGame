...
var TotalPlayers = 6;
//gameId you created or joined now.
var GameId = 0;
var IsDealer = true;
var Sno = 3;
var GameCode = "";
...
var UserIdentity = {
    UserName: '', //UserName you input when you created or joined game.
    UniqueId: '', //UniqueId that generated by guid from the server-side.
    ConnectionId: '', //ConnectionId to signialR.
    GameCode: '', //GameCode you created or joined now.
};
...
$(document).on("click", ".BtnJoin", function () {
    if ($('#UserName1').val() == "" || $('#GameCode1').val() == "") {
        ...
        JoinGame($('#UserName1').val(), connection.connectionId, $('#GameCode1').val());
    }
});
...
$(document).on("click", ".Ante", function () {
    ...
    if (GameHash.ActivePlayers.filter(x => x.IsFolded === "N" && x.IsDealer === "N").length > 0) {
        ...
        //Set LastActionPerformed

        //If LastActionPerformed of current player's Sno already set, just change LastActionPerformed's action
        SetLastActionPerformed(Sno, {
            PlayerId: GameHash.ActivePlayers.filter(x => x.Sno === Sno)[0].PlayerId,
            PlayerSno: Sno,
            Action: "Ante: " + anteValue,
            Amount: anteValue
...
$(document).on("click", ".PassDeal", function () {
    ...
    //Add Player List to pass Deal (except current you and not folded also)
...
$(document).on("click", ".AddToPot", function () {
    ...
    try {
        ...
        if (betamount < 1) {
            ...
            //Change GameHash's ActivePlayer's PlayerAmount (previous PlayerAmount - betamount)
            ...
            //Set CurrentPlayer's LastAction as " added to pot"
            SetLastActionPerformed(Sno, {
                PlayerId: GameHash.ActivePlayers.filter(x => x.Sno === Sno)[0].PlayerId,
                PlayerSno: Sno,
                Action: msg1,
            ...
            try {
                var msg = GameHash.Steps[GameHash.Steps.length - 1].Step.Action + " - by " + GetUserNameFromLocalStorage() + " Amount : " + betamount;
                SendNotification(GameHash.GameId, "", msg);
                UpdateGameHash(GameHash.GameId);
            } catch (err) {
                console.log(err);
...
$(document).on("click", ".Take", function () {
    ...
    try {
        ...
        */
        var takeamount = ($('#BetTakeValue').val() === "" || $('#BetTakeValue').val() === "0") ? GameHash.PotSize : $('#BetTakeValue').val();
        ...
        if (takeamount > GameHash.PotSize || takeamount < 0) {
            ...
            //Update ActivePlayers PlayerAmount( += takeamount)
            var playeramount = GameHash.ActivePlayers.filter(x => x.Sno === Sno)[0].PlayerAmount;
            GameHash.ActivePlayers.filter(x => x.Sno == Sno)[0].PlayerAmount = parseFloat(playeramount) + parseFloat(takeamount);

            //Remove takeamount from the PotSize
            ...
            //Send Notification
            try {
                var msg = GameHash.Steps[GameHash.Steps.length - 1].Step.Action + " - by " + GetUserNameFromLocalStorage() + " Amount : " + takeamount;
                SendNotification(GameHash.GameId, "", msg);
            } catch (err) {
                console.log(err);
            }
...
$(document).on("click", ".BtnSitOut", function () {
    try {
        ...
        if (GameHash.ActivePlayers.filter(x => x.Sno > Sno && x.IsFolded === "N").length == 0)
        {
            ...
            GameHash.BetStatus = GetBetStatus(GameHash.ActivePlayers.filter(x => x.IsFolded === "N")[0].Sno);
        ...
        //there is at least one player folded that joined in the game after you.
            ...
            GameHash.BetStatus = GetBetStatus(GameHash.ActivePlayers.filter(x => x.Sno > Sno && x.IsFolded === "N")[0].Sno);
        ...
        try {
            //Send Notification
            var msg = GameHash.Steps[GameHash.Steps.length - 1].Step.Action + " - by " + GetUserNameFromLocalStorage();
            SendNotification(GameHash.GameId, "", msg);
        } catch (err) {
            console.log(err);
    ...
    try {
        ...
        if (GameHash.ActivePlayers.filter(x => x.Sno === Sno).length == 1) {
            ...
            if (GameHash.ActivePlayers.filter(x => x.Sno === Sno)[0].IsDealer == "Y") {
                ...
                if (GameHash.ActivePlayers.filter(y => y.IsFolded === "N" && y.Sno > Sno).length > 0) {
                ...
                else if (GameHash.ActivePlayers.filter(y => y.IsFolded === "N").length > 0) {
...
$(document).on("click", ".Bet", function () {
    ...
    try {
        ...
        //get betamount
        var betamount = $('#BetTakeValue').val();
        ...
        //if betamount is suitable.
            ...
            //decrease Total player amount
            GameHash.ActivePlayers.filter(x => x.Sno === Sno)[0].PlayerAmount = parseFloat(playeramount) - parseFloat(betamount);

            //Increase Total PotSize of the Game.
            GameHash.PotSize = parseFloat(GameHash.PotSize) + parseFloat(betamount);
...
$(document).on("click", ".Call", function () {
    ...
    if (GameHash.CurrentBet === 0) {
        ...
        //Update CurrentRoundStatus of current player(+= current)
        var currentplayerbet = betamount - parseFloat(GameHash.ActivePlayers.filter(x => x.Sno === Sno)[0].CurrentRoundStatus);
        GameHash.ActivePlayers.filter(x => x.Sno === Sno)[0].CurrentRoundStatus = betamount;

        var msg1 = "called with " + currentplayerbet;

        //Add Total PotSize and Update PlayerAmount of current player.
        IncreaseTotalPotSizeAndUpdatePlayerAmount(Sno, currentplayerbet);
        ...
        try {
            //Send Notification
            var msg = GameHash.Steps[GameHash.Steps.length - 1].Step.Action + " - by " + GetUserNameFromLocalStorage() + " Amount : " + currentplayerbet;
            SendNotification(GameHash.GameId, "", msg);
        } catch (err) {
            console.log(err);
        }
...
$(document).on("click", ".Fold", function () {
    ...
    try {
        ...
        //If there is somebody not folded and joined after current player
            ...
            //Get first unfolded player
            var tempActivePlayer = GameHash.ActivePlayers.sort(function (a, b) {
                return a.Sno - b.Sno
            }).filter(x => x.IsFolded === "N")[0];

            tempActivePlayer.IsCurrent = "Y";

            //Update Current BetStatus
            GameHash.BetStatus = GetBetStatus(tempActivePlayer.Sno);
        }

        //Otherwise
        else {

            //Get first unfolded player joined after me as Current
            var tempActivePlayer = GameHash.ActivePlayers.sort(function (a, b) {
                return a.Sno - b.Sno
            }).filter(x => x.Sno > Sno && x.IsFolded === "N")[0];

            tempActivePlayer.IsCurrent = "Y";

            //Set Current Bet Status
            GameHash.BetStatus = GetBetStatus(tempActivePlayer.Sno);
...
$(document).on("click", ".Pass", function () {
    try {
        ...
        if ((parseFloat(GameHash.CurrentBet) - parseFloat(GameHash.ActivePlayers.filter(x => x.Sno === Sno)[0].CurrentRoundStatus)) != 0) {
            ...
            try {
                //Send notification
                var msg = GameHash.Steps[GameHash.Steps.length - 1].Step.Action + " - by " + GetUserNameFromLocalStorage() + " Amount : " + 0;
                SendNotification(GameHash.GameId, "", msg);
            } catch (err) {
                console.log(err);
                ExceptionLogging(err);
...
/*
 * Right now, there is no "Shuffle" button.
*/
$(document).on("click", ".BtnShuffle", function () {
    ...
    try {
        var msg = "Deck shuffled - by " + GetUserNameFromLocalStorage();
        SendNotification(GameHash.GameId, "", msg);
    } catch (err) {
        console.log(err);
        ExceptionLogging(err);
...
function PassDealPlayer(newdealerSno) {
    ...
    //set current
    if (GameHash.ActivePlayers.filter(x => x.Sno > newdealerSno).length == 0) {
...
function addPlayerCardAndUpdateGameHashDeck(currentPlayerNo) {
    ...
    GameHash.ActivePlayers.filter(x => x.Sno === currentPlayerNo)[0].PlayerCards.push({
...
function SetLastActionPerformed(current_sno, action) {
    ...
    if (GameHash.LastActionPerformed.filter(x => x.PlayerSno === current_sno).length === 1)
        GameHash.LastActionPerformed.filter(x => x.PlayerSno === current_sno)[0].Action = action
...
function SetFirstUnfoldedPlayerAsDealer(Sno = -1)
{
    ...
    //set first player after me that not folded as dealer
    GameHash.ActivePlayers.sort(function (a, b) {
        return a.Sno - b.Sno
...
/**
 * Increase TotalPotSize by bet
 * @param
 * currentSno: current player's Sno
 * bet: betting amount
*/
function IncreaseTotalPotSizeAndUpdatePlayerAmount(currentSno, bet) {
    var playeramount = GameHash.ActivePlayers.filter(x => x.Sno === currentSno)[0].PlayerAmount;
    GameHash.ActivePlayers.filter(x => x.Sno === currentSno)[0].PlayerAmount = parseFloat(playeramount) - parseFloat(bet);
    GameHash.PotSize = parseFloat(GameHash.PotSize) + parseFloat(bet);
}
...
function passCardToPlayer(fromPlayer, toPlayer, _CardValue)
{
    ...
    GameHash.ActivePlayers.filter(x => x.Sno === fromPlayer)[0].PlayerCards = GameHash.ActivePlayers.filter(x => x.Sno === fromPlayer)[0].PlayerCards.filter(y => y.Value !== _CardValue);
}
    ...
    try {
        ...
        */
        ...
        GameHash.ActivePlayers.push({
            PlayerId: GeneratePlayerId($('#UserName').val(), GetUserId()),
            PlayerName: "P1",
            PlayerCards: [],
            PlayerAmount: 0, // taken - bet = amount
            ConnectionId: connection.connectionId,
         ...
         **/
        model = {
            "UserId": GeneratePlayerId($('#UserName').val(), GetUserId()),
            "GameCode": GameCode,
            "GameHash": JSON.stringify(GameHash),
            "ConnectionId": connection.connectionId,
            "PlayerUniqueId": GetUserId(),
...
function StartGame() {
    try {
        ...
        //Update View for updated GameHash
        UpdateView();
...
function JoinGame(UserName, ConnectionId, GameCode) {
    ...
    model = {
        "UserId": GeneratePlayerId(UserName, GetUserId()),
        "GameCode": GameCode,
        "GameHash": JSON.stringify(GameHash),
        "ConnectionId": connection.connectionId,
        "PlayerUniqueId": GetUserId(),
    ...
    $.ajax({
        ...
        /*else if (result == "101") {
            ...
            //set userid1 as playerId of UserName + "pk2" + UserId
            var userid1 = GeneratePlayerId(UserName, GetUserId());

            //localstorage
            setLocalStorage(GameCode, userid1);
            ...
            */
            if (GameHash.ActivePlayers.length < 6 && GameHash.ActivePlayers.filter(x => x.PlayerUniqueId === GetUserId()).length == 0) {
            ...
            // When there are less than 5 ActivePlayers and you already joined and not foled there and also you added to the ContinuityPlayer list
            else if (GameHash.ActivePlayers.length < 6 && GameHash.ActivePlayers.filter(x => x.PlayerUniqueId === GetUserId() && x.IsFolded === "N").length === 1 && GameHash.ContinuityPlayers.filter(x => x.PlayerUniqueId === GetUserId()).length === 1) {

                // set Sno as your son in the Game.
                Sno = GameHash.ActivePlayers.filter(x => x.PlayerUniqueId === GetUserId())[0].Sno;
                var IsCurrent1 = "N";
                ...
                */
                var ExistingPlayer1 = GameHash.ActivePlayers.filter(x => x.PlayerUniqueId === GetUserId())[0];
                ExistingPlayer1.ConnectionId = connection.connectionId;
                GameHash.ContinuityPlayers = GameHash.ContinuityPlayers.filter(x => x.PlayerUniqueId !== GetUserId());
                GameHash.ActivePlayers.filter(x => x.PlayerUniqueId === GetUserId())[0] = ExistingPlayer1;
            ...
            else if (GameHash.ActivePlayers.filter(x => x.PlayerUniqueId === GetUserId() && x.IsFolded === "Y").length === 1) {

                // if you didn't added to ContinuityPlayers list, add you there.
                if (GameHash.ContinuityPlayers.filter(x => x.PlayerUniqueId === GetUserId()).length === 0)
                    GameHash.ContinuityPlayers.push(GameHash.ActivePlayers.filter(x => x.PlayerUniqueId === GetUserId())[0]);
        ...
        if (PlayerId === -1) {
            ...
            for (var i = 0; i < NumOfCard; i++) {
                for (var j = 0; j < NumberOfPlayer; j++) {
                    var currentPlayerNo = j + 1;

                    //If j + 1 player is not foleded
                    if (GameHash.ActivePlayers.filter(x => x.Sno === currentPlayerNo)[0].IsFolded === "N") {
...
function OnPlayerAction() {
    ...
    try {
        ...
        if (Sno === DealerSno) {
            ...
            if (GameHash.ActivePlayers.filter(y => y.IsFolded === "N" && y.Sno === DealerSno).length === 0) {
                ...
                if (GameHash.ActivePlayers.sort(function (a, b) {
                    ...
                    setFirstUnfoldedPlayerAsCurrent(SnoTemp);
                    ...
                    SetFirstUnfoldedPlayerAsDealer(DealerSno);

                    if (GameHash.ActivePlayers.filter(y => y.IsFolded === "N" && y.Sno > SnoTemp).length > 0)
                    ...
                    GameHash.ActivePlayers.filter(y => y.Sno === SnoTemp)[0].IsCurrent = "Y";

                    GameHash.BetStatus = GetBetStatus(GameHash.ActivePlayers.filter(y => y.Sno === SnoTemp)[0].Sno);
    ...
    //If you select specific player
        ...
        $.each(GameHash.ActivePlayers.filter(x => x.Sno === Sno), function (i, obj) {
            ...
            for (var count = 0; count < $selectedCard.length; count++) {
                ...
                //Add Card to PlayerSno, remove Card from Sno
...
function startConnectionWithUserIdentity(UserIdentity) {
    var url = "/GameClass?UserIdentity=" + UserIdentity + "";
